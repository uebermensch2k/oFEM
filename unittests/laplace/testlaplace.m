...
function testP1StiffnessAndMass2D(testCase)
    ...
    % optimized
    Sopt = stiffness_matrix_P1_2D(mesh.co,mesh.el,mat);
    Mopt =      mass_matrix_P1_2D(mesh.co,mesh.el);
    ...
    % fastFEM
    [SfastFEM,areas] = stifness_matrixP1_2D(mesh.el,mesh.co);
    MfastFEM         =     mass_matrixP1_2D(mesh.el,areas  );
...
%     mat = ones(size(mesh.el, 1),1);
    ...
    Mopt =      mass_matrix_P1_3D(mesh.co,mesh.el);

...
classdef PhiAxi < handle
    % ofem.PhiAxi implements the assemble routines and the interface for
    % solving an axis symmetric problem for the Phi symmetry. Given a
    % problem in cylindrical coordianets (\rho, \phi, z) and assuming
    % symmetry in \phi, i.e.,
    %    \vec{A}(\rho, \phi, z) = A(\rho, z)\vec{e}_{\phi}
    % The solution
    %
    %  \Delta \vec{A}(\rho, \phi, z) - c \vec{A}(\rho, \phi, z) = f(\rho, \phi, z)
    %
    %
    % can be computed. The laplace opetartor in cylindircal coordinates is
    % considered and the instead of dividing by \rho^2 all quantyties are
    % multiplied with \rho.
    %
    ...
    methods(Access=protected,Static)
        ...
        %%
        function S=stiffness(A,DinvT,detD,dphi,pipj, w,l,el,co)
    ...
    methods
        %%
        function obj=PhiAxi(mesh,fe,qr)
        ...
        function [asm,info,aux] = assemble(obj,varargin)
            ...
            if intvol==1
                ...
                for i=1:Np
                    ...
                    if opt.S==1
                        if iscell(opt.A)
                            aux.S{i} = obj.stiffness(opt.A{i},DinvTLoc,detDLoc,dphi,pipj, w,l,elemsLoc,obj.mesh.co);
                        else
                            aux.S{i} = obj.stiffness(opt.A,DinvTLoc,detDLoc,dphi,pipj,w,l,elemsLoc,obj.mesh.co);
                        end
                        S = S + aux.S{i};
                    end
            ...
            %% Dirichlet data
            DOFs = 1:Nc; % NOTE: this is only valid for P1 elements => needs an update
            if intdiri==1
                for i=1:Ndiri

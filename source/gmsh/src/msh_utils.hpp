...
//    trimmer >> rkString;
  ...
  inline
  bool
  getline_checked(std::ifstream &rkMshFileStream, std::string &rkLine)
  {
    getline(rkMshFileStream, rkLine);

    /* check stream state */
    if (!rkMshFileStream.good())
    {
      if (rkMshFileStream.eof())
        return false;
      else
...
//  }
  ...
  inline
  void
  open_msh_file(const mxArray *cpkMshFileName,
                std::ifstream &rkMshFileStream)
  {
    /* read file name */

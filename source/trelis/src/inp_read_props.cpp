...
const char * const g_cpccFunctionIdStr = "MATLAB:mexfunction:inp_read_props";
...
//    mexPrintf("header: %s.\n",inp_nodes_section::ms_SectionHeader.c_str());
    ...
    if (!kLine.compare(inp_properties_section::ms_SectionHeader))
    {
      try {
//        mexPrintf("stream position: %d.\n",(int) kInpFileStream.tellg());
        plhs[0] = properties_section.scan(kInpFileStream);
      }
      catch (string err) {
        PrintErrAndExit((err+"\n").c_str());
      }

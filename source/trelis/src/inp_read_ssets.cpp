...
const char * const g_cpccFunctionIdStr = "MATLAB:mexfunction:inp_read_ssets";
...
//    mexPrintf("header: %s.\n",inp_nodes_section::ms_SectionHeader.c_str());
    ...
    if (!kLine.compare(inp_sidesets_section::ms_SectionHeader))
    {
      try {
//        mexPrintf("stream position: %d.\n",(int) kInpFileStream.tellg());
        plhs[0] = sidesets_section.scan(kInpFileStream);
      }
      catch (string err) {
        PrintErrAndExit((err+"\n").c_str());
      }
